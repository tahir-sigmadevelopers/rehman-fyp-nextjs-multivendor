...
export async function GET() {
  try {
    ...
    for (const pageData of webPages) {
      try {
        ...
        if (existingPage) {
          ...
          results.push({ title: pageData.title, status: 'updated' })
        } else {
          // Create new page
          await WebPage.create(pageData)
          results.push({ title: pageData.title, status: 'created' })
        }
      } catch (error) {
        results.push({ 
          title: pageData.title, 
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'Web pages processed successfully', 
      results 
    })
  } catch (error) {
    return NextResponse.json({ 
      success: false, 
      message: 'Failed to process web pages', 
      error: error instanceof Error ? error.message : String(error) 
    }, { status: 500 })
  }
} 