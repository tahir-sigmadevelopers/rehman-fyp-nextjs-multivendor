const { connectToDatabase } = require('../lib/db/index');
...
// Import the WebPage model dynamically to avoid issues with Next.js server components
async function run() {
  try {
    ...
    // Import the model after connecting to the database
...
- **Hours:** Monday to Friday, 9:00 AM - 6:00 PM EST`,
    ...
    console.log('Creating web pages...');
    ...
    for (const pageData of webPages) {
      try {
        ...
        if (existingPage) {
          console.log(`Page "${pageData.title}" already exists. Updating content...`);
          await WebPage.findByIdAndUpdate(existingPage._id, {
            title: pageData.title,
            content: pageData.content,
            isPublished: pageData.isPublished
          });
          console.log(`Updated "${pageData.title}" page successfully!`);
        } else {
          // Create the page
        ...
        console.error(`Error processing page "${pageData.title}":`, error);
      }
    }
